...
function assumeEnvPrecedence(envValue, defaultValue) {
  if (isSet(envValue)) {
    return envValue;
  }
  return defaultValue;
...
async function setupOptions() {
  ...
  if (await yesno({ question: `Would you like to set drobadi config file (${CONFIG_FILE_FRIENDLY}) ? (y/n)`, defaultValue: null })) {
...
async function setupTargetConfigFile(defaultConfigFile) {
  const questions = [{type: 'input',name: 'configLocation', message: `Dropbox target config file (default is ${defaultConfigFile})`}];
  return new Promise((resolve, reject)=>  {inquirer.prompt(questions).then(answers => {resolve(answers.configLocation)});});
}
